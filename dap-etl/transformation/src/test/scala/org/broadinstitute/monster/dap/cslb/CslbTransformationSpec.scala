...
class CslbTransformationSpec extends AnyFlatSpec {
  ...
  private val exampleCslbFields = Map[String, Array[String]](
    ...
    "redcap_event_name" -> Array("annual_2020_arm_1"),
    "cslb_pace" -> Array("4"),
    "cslb_stare" -> Array("4"),
    "cslb_stuck" -> Array("4"),
  ...
  it should "raise when date is invalid" in {
    val invalidDateRecord = RawRecord(
      id = 1,
      Map("cslb_date" -> Array("2020-142-124"), "redcap_event_name" -> Array("annual_2020_arm_1"))
    )
  ...
  it should "raise when redcap_event_name is not provided" in {
    val invalidEventRecord = RawRecord(id = 1, Map("cslb_date" -> Array("2020-11-19")))
    assertThrows[IllegalStateException] {
      CslbTransformations.mapCslbData(invalidEventRecord)
    }
  }

  it should "raise when redcap_event_name is invalid" in {
    val invalidEventRecord = RawRecord(
      id = 1,
      Map("cslb_date" -> Array("2020-11-19"), "redcap_event_name" -> Array("dec2019_arm_1"))
    )
    assertThrows[IllegalStateException] {
      CslbTransformations.mapCslbData(invalidEventRecord)
    }
  ...
  it should "correctly maps cslb values" in {
    ...
    output.cslbYear shouldBe 2020L
