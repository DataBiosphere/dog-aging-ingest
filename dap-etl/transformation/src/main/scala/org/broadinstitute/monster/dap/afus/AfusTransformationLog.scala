...
abstract class AFUSTransformationLog extends Exception {
  val jsonMsg: Obj

  def log(implicit logger: Logger): Unit
}

class AFUSTransformationError(msg: String) extends AFUSTransformationLog {
...
class AFUSTransformationWarning(msg: String) extends AFUSTransformationLog {

  def log(implicit logger: Logger): Unit = {
    logger.error(jsonMsg.toString())
    counter("afus", "afus_transformation_warning").inc()
  }
...
case class MissingOwnerIdError(msg: String) extends AFUSTransformationError(msg)
