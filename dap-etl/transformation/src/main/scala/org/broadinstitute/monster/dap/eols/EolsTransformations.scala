...
object EolsTransformations {
  ...
  /** Parse all eols related fields out of a RedCap record.
    ...
    */
  def mapEols(rawRecord: RawRecord): Eols = {
    val dogId = rawRecord.id
    Eols(
      dogId = dogId,
      // todo: some of these fields actually have gating logic - refactor this
      eolWillingToComplete = rawRecord.getOptionalBoolean("eol_willing_to_complete"),
      ...
      eolCauseOfDeathPrimaryOtherDescription = rawRecord.getOptional("eol_cause1_death_specify"),
      eolCauseOfDeathSecondary = rawRecord.getOptionalNumber("eol_cause2_death"),
      eolCauseOfDeathSecondaryOtherDescription = rawRecord.getOptional("eol_cause2_death_specify"),
