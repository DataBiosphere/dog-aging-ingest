...
from dagster.core.definitions.pipeline_sensor import pipeline_failure_sensor, run_status_sensor, RunStatusSensorContext
...
def _slack_on_pipeline_status(pipeline_run: PipelineRun, status: PipelineRunStatus) -> None:
    ...
    lines = [
        f"Dog Aging Pipeline entered state {status.value}",
