...
# INVOCATION SYNTAX: ./hack/manual_refresh.sh [-e env] [-t tsv_output_path] [-t start_date] [-t end_date] [-o refresh_subdirectory] [-u] [pipelines...]
...
## start_date: adds the startTime parameter to the extraction sbt calls for each pipeline, defaults to the Epoch values specified in each extraction pipeline
## end_date: adds the endTime parameter to the extraction sbt calls for each pipeline, defaults to the current date
...
while getopts "e:t:s:d:o:u" opt; do
    case "$opt" in
    e)
        ENV="$OPTARG"
        ;;
...
# composes sbt arguments if specific date arguments were provided
if [[ "$START_DATE" ]]; then
  START_TIME=" --startTime=$START_DATE"
fi

if [[ "$END_DATE" ]]; then
  END_TIME=" --endTime=$END_DATE"
fi
